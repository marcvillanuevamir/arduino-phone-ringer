...
// timing
unsigned long previousMillis = 0;  // global millis (not in use)
...
static void longPressHandler(uint8_t btnId, uint8_t btnState) {
  if (btnState == BTN_PRESSED) {
    ...
    // After it has been held down a long time and released
    if (!useShort) {
      scene = 0;
      return;
    }
...
void loop() {
  ...
  if (Serial.available()) {
    switch (Serial.read()) {
      case '1': ring1 = true; break;
      case '2': ring2 = true; break;
      case '3': ring3 = true; break;
      case 'q': ring1 = false; break;
      case 'w': ring2 = false; break;
      case 'e': ring3 = false; break;
      case 's':  // graceful stop
        stopall();
        break;
      case 'f':  // all in sync
        ring1 = true;
